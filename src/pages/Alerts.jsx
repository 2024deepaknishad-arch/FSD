...
export default function Alerts() {
  const { alerts, highRiskCount, mediumRiskCount, loading, error, resolveAlert } = useAlerts();
  ...
  return (
    <Layout title="Security Alerts" statusText={highRiskCount ? `${highRiskCount} high-risk alert` : 'No high-risk alerts'} statusColor={highRiskCount ? 'red' : 'green'}>
      ...
      <div className="space-y-3">
        {loading && <p className="text-dim text-sm">Loading alerts…</p>}
        {error && <p className="text-red text-sm">{error}</p>}
        {!loading && alerts.length === 0 && <p className="text-dim text-sm">All clear — no active alerts.</p>}
        {alerts.map((a) => {
