...
export default function AuditLogs() {
  ...
  const { logs, loading, error, addAuditLog } = useAuditLogs({ search, status });
  ...
  return (
    <Layout title="Audit Logs">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button onClick={simulateActivity} className="bg-cyan text-bg text-sm font-semibold px-4 py-2 rounded-md hover:bg-cyan/90 transition">
          Simulate Activity
        </button>
      </div>
      ...
      <div className="bg-surface border border-border rounded-lg overflow-hidden">
        ...
        {error && <p className="px-5 py-3 text-xs text-red border-b border-border">{error}</p>}
